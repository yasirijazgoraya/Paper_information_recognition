"""
Layer 1 Scoring v4 — Field-Level F1 with Precision and Confidence Intervals

Builds directly on v3. Same Fix A / Fix B / Fix C matching logic, same file
layout, same loaders. What changes:

  • SROIE and CORD now report PRECISION and F1, not recall only.
      - v3 reported field_recall = (fields found) / (fields present in GT).
        A backend that dumps the whole page could score high on recall while
        emitting huge amounts of noise. v4 adds precision so that over-emission
        is penalised, and reports the harmonic mean (F1) as the headline.
  • SROIE: one value per field is selected from the prediction before matching,
    so per-field precision/recall/F1 are well-defined (METRICS.md §7 caveat).
  • CORD: precision is computed against the predicted field candidates, giving a
    symmetric F1 (METRICS.md §7 "Precision on CORD" future-work item, now done).
  • Bootstrap 95% confidence intervals on every headline metric (METRICS.md §7).
  • Field-level F1 is the HEADLINE column; Token F1 is retained but demoted to a
    supporting column, because Token F1 understates KIE quality.

FUNSD scoring is unchanged from v3 (answer-token recall + Q->A pair recall);
its strict pair recall is already the right diagnostic.
...
Outputs:
    results/metrics/full_comparison_v4.csv
    results/metrics/full_comparison_v4.md
    results/metrics/<dataset>_<model>_v4.csv         (per-doc detail)
    results/metrics/<dataset>_<model>_failures_v4.csv (per-doc failures)
"""
...
def main():
    ...
    print("=" * 78)
    print(" " * 14 + "LAYER 1 SCORING v4 — Field-Level F1 + Precision + CIs")
    print("=" * 78)
    ...
    for dataset in DATASETS:
        ...
        for model in MODELS:
            ...
            if df is None:
                ...
                continue
            df.to_csv(METRICS_DIR / f"{dataset.lower()}_{model}_v4.csv", index=False)
            if failures:
                pd.DataFrame(failures).to_csv(
                    METRICS_DIR / f"{dataset.lower()}_{model}_failures_v4.csv", index=False)
    ...
    pd.DataFrame(summary_rows).to_csv(METRICS_DIR / "full_comparison_v4.csv", index=False)

    # Markdown report
    md = ["# Layer 1 — Field-Level Comparison v4\n",
    ...
    (METRICS_DIR / "full_comparison_v4.md").write_text("\n".join(md) + "\n", encoding="utf-8")

    print(f"\nCSV: {METRICS_DIR / 'full_comparison_v4.csv'}")
    print(f"MD:  {METRICS_DIR / 'full_comparison_v4.md'}")
